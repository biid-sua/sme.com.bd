...
$qry = "SELECT * FROM ".$db_prefix."photos $filter ORDER BY id DESC LIMIT ".(($ppage-1)*$pcount).", ".$pcount;
$res = mysql_query($qry);
?>
<div>
<p>
		...
		?>
			<td align="center">
				<table border="0" cellpadding="0" cellspacing="0" width="184">
					...
					<tr>
						<td align="left" style="padding-top:5px" width="25" height="30"><input type="checkbox" class="styled" title="lightbox" name="addlightbox" value="<?php echo $rs['id'];?>" <?php echo (in_array($rs['id'], $lightbox)? 'checked' : '');?>></td>
...
else
{
	...
	$qry = "SELECT p.*, c.cat_title FROM photos as p, categories as c WHERE p.id=$pid AND p.image_cat=c.id";
	$res = mysql_query($qry);
	$rs = mysql_fetch_assoc($res);
...
<p>
	...
	elseif($_GET['r'] == 'lightbox')
		$totalcount = count($lightbox);
	
	//Get image position in the records
	$recordpos = 0;
	if(empty($_GET['c']) && empty($_GET['r']))
	{
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE image_cat=".$rs['image_cat']." ORDER BY id DESC";
	...
	elseif(!empty($_GET['c']))
	{
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE image_cat=".mysql_real_escape_string($_GET['c'])." ORDER BY id DESC";
		$respos = mysql_query($qrypos);
		$icount = 0;
		while($rspos = mysql_fetch_assoc($respos))
		{
	...
	if(empty($_GET['c']) && empty($_GET['r']))
	{
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE id <= $pid AND image_cat=".$rs['image_cat']." ORDER BY image_order DESC LIMIT 2";
		$respos = mysql_query($qrypos);
		...
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE id >= $pid AND image_cat=".$rs['image_cat']." ORDER BY image_order ASC LIMIT 2";
		$respos = mysql_query($qrypos);
		if(mysql_num_rows($respos) > 1)
		{
			mysql_data_seek($respos, 1);
	...
	elseif(!empty($_GET['c']))
	{
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE id <= $pid AND image_cat=".mysql_real_escape_string($_GET['c'])." ORDER BY image_order DESC LIMIT 2";
		$respos = mysql_query($qrypos);
		if(mysql_num_rows($respos) > 1)
		{
			mysql_data_seek($respos, 1);
		...
		$qrypos = "SELECT * FROM ".$db_prefix."photos WHERE id >= $pid AND image_cat=".mysql_real_escape_string($_GET['c'])." ORDER BY image_order ASC LIMIT 2";
		$respos = mysql_query($qrypos);
		if(mysql_num_rows($respos) > 1)
		{
			mysql_data_seek($respos, 1);
	...
	elseif($_GET['r'] == 'lightbox')
	{
		$nextpos = $lightbox[$recordpos];
		$prevpos = $lightbox[$recordpos-2];
	}
	?>
	<table border="0" cellpadding="0" cellspacing="0" style="position:relative;left:14px;">
		...
		<tr>
			<td>
				<div id="imgdetail"><img src="<?php echo $imguploadpath.$rs['image_name'];?>" border="0" style="border:1px solid #cccccc"></div>
			</td>
		</tr>
		...
		<tr><td>
			<input type="button" class="button" name="lightbox" id="lightbox" onClick="toggleLightbox()" value="<?php echo (in_array($pid, $lightbox)? 'Remove from lightbox' : 'Add to lightbox');?>">&nbsp;&nbsp;<input type="button" class="button" name="cart" id="cart" onClick="toggleCart()" value="<?php echo (in_array($pid, $cart)? 'Remove from cart' : 'Add to cart');?>">
		</td></tr>
	</table>
</p>
<script language="javascript">
...
var inLightbox = <?php echo (in_array($pid, $lightbox)?1:0);?>;
